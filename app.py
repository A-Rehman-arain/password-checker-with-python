...
def evaluate_password_strength(password):
    ...
    # Check common patterns (case-insensitive)
    common_patterns = ['password', '123', 'qwerty', 'admin', 'letmein']
    if any(pattern in password.lower() for pattern in common_patterns):
        feedback.append("Avoid common words or sequences")
...
password_length = st.slider("Select password length", min_value=8, max_value=20, value=14)

if st.button("Generate Strong Password 🔑"):
    st.session_state.generated_pw = generate_strong_password(password_length)
...
if st.session_state.generated_pw:
    ...
    if st.button("Copy to Clipboard 📋"):
        try:
            ...
            st.error("Failed to copy to clipboard. Please manually copy the password.")
